...
if __name__ == "__main__":
    ...
    for file in filename_tijd:
        filenames = []
        for run in range(20):
            if not file_exists(f"{run}_{file[0]}"):
                filenames.append(f"{run}_{file[0]}")
        if (len(filenames) != 0):
            objects = parse_gcode("../NNA/" + file[0])
            ...
            for filename in filenames:
                ...
                write_route_gcode_to_file(objects, permutation, filename)
